...
///
/// Construct with [CareRoundsDatabase.open] in production — it lazily opens the
/// SQLite file (`carerounds.sqlite`) under the platform's app-documents
/// directory.
///
...
class CareRoundsDatabase extends _$CareRoundsDatabase {
  CareRoundsDatabase(super.executor) : _isShared = false;
  ...
  CareRoundsDatabase._shared(super.executor) : _isShared = true;
  ...
  static CareRoundsDatabase? _sharedInstance;

  /// Opens the on-device SQLite file named `carerounds.sqlite` in the
  ...
  factory CareRoundsDatabase.open({
    QueryExecutor Function()? createExecutor,
  }) =>
      _sharedInstance ??= CareRoundsDatabase._shared(
        (createExecutor ??
            () => localFileExecutor(
  ...
  @visibleForTesting
  static CareRoundsDatabase openShared(QueryExecutor executor) =>
      _sharedInstance ??= CareRoundsDatabase._shared(executor);
  ...
  factory CareRoundsDatabase.testInstance() =>
