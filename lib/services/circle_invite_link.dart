...
/// Deep-link host for a care-circle invite: `carerounds://join/<token>`.
/// This is the LINK channel parallel to the QR's `carerounds:circle:<token>`
...
/// app via the `carerounds://join/<token>` deep link.
...
///   * `carerounds://join/<token>`   (host `join`, first path segment)
///   * `carerounds:circle:<token>`   (the QR payload scheme)
String? parseCircleInviteToken(Uri uri) {
  // QR-style opaque scheme: `carerounds:circle:<token>`. Uri parses this as
  // scheme=carerounds, path=`circle:<token>` (no authority), so match on the
  ...
  // Link-style: `carerounds://join/<token>`.
  if (uri.scheme == 'carerounds' && uri.host == circleJoinHost) {
