...
/// (BUILD_SPEC.md Phase 9.2 — `carerounds/tts`).
///
...
const String bundledTtsChannelName = 'carerounds/tts';
...
class BundledTTSProvider implements TTSProvider {
  ...
  ///
  /// Probes the `carerounds/tts` MethodChannel by invoking `probe`.
...
/// `carerounds.tts` logger name — matches what `flutter run` /
...
void _defaultTtsWarn(String message) {
  developer.log(message, name: 'carerounds.tts', level: 900);
}
