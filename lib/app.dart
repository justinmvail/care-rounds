...
/// Root widget. Wires MaterialApp.router to `careroundsLightTheme`
/// + `careroundsDarkTheme` with system-mode fallback.
///
/// Stateful so the GoRouter is constructed once and survives rebuilds
/// — GoRouter holds navigation state internally.
class CareRoundsApp extends ConsumerStatefulWidget {
  const CareRoundsApp({super.key, this.router});

  /// Optional injected router for tests. Defaults to the production
  /// `careroundsRouterProvider`, which wires the auth + onboarding
  /// redirect (BUILD_SPEC.md §5.11 + §5.12).
  final GoRouter? router;

  @override
  ConsumerState<CareRoundsApp> createState() => _CareRoundsAppState();
}

class _CareRoundsAppState extends ConsumerState<CareRoundsApp> {
  late final GoRouter _router =
      widget.router ?? ref.read(careroundsRouterProvider);

  // Care Circle invite LINKS (2026-06-08). Listens for incoming
  // `carerounds://join/<token>` deep links (cold-start + warm) and a
  ...
  void _applyOutcome(CircleJoinOutcome outcome) {
    ...
    switch (outcome) {
      case CircleJoinConfirmationRequired(:final token):
        // NEVER auto-join from a link: a tapped carerounds://join/<x>
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp.router(
      ...
      // active CareRoundsColors extension), so both palettes render the
