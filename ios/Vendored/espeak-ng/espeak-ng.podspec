...
Pod::Spec.new do |s|
  s.name             = 'espeak-ng'
  s.version          = '1.52.0'
  s.summary          = 'Vendored espeak-ng for CareRounds iOS TTS phonemizer.'
  s.description      = <<-DESC
  ...
  s.author           = { 'CareRounds' => 'team@carerounds.app' }
  s.source           = { :path => '.' }
  s.platform         = :ios, '16.0'
  ...
  # bridging header cannot resolve espeak at all, `CAREROUNDS_HAS_ESPEAK_NG`
  ...
  s.user_target_xcconfig = {
    ...
    # `#if CAREROUNDS_HAS_ESPEAK_NG`. That is a SWIFT compilation condition —
    # and Swift does NOT see C preprocessor macros from the bridging header. The
    # `#define CAREROUNDS_HAS_ESPEAK_NG 1` there is invisible to it; an unknown
    ...
    'SWIFT_ACTIVE_COMPILATION_CONDITIONS' => '$(inherited) CAREROUNDS_HAS_ESPEAK_NG',
  }
end
