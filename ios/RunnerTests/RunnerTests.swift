...
class RunnerTests: XCTestCase {
    ...
    ///
    /// Skips when `CAREROUNDS_HAS_ESPEAK_NG` is 0 (no vendored sources)
    ...
    ///
    /// Skips when `CAREROUNDS_HAS_ESPEAK_NG` is 0 — that's the state
    ...
    /// carerounds-tts-samples/<voice>/`. The operator pulls those
    ...
    func testRegenerateAudioQualitySamples() throws {
        ...
        let outDir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("carerounds-tts-samples")
