...
// BUILD_SPEC.md Phase 9.3 — `carerounds/tts` MethodChannel handler.
//
...
enum TTSBridge {
    ...
    static let channelName = "carerounds/tts"
    ...
    static func register(with registry: FlutterPluginRegistry) {
        guard let registrar = registry.registrar(forPlugin: "CareRoundsBundledTTS") else {
            return
        }
...
final class TTSEngine {
    ...
    private let workQueue = DispatchQueue(label: "carerounds.tts.work", qos: .userInitiated)
...
/// short-circuits and `CAREROUNDS_HAS_ESPEAK_NG` Swift flag is unset),
