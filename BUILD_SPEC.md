...
>
> This spec describes the **original** product: "CareRounds," a
...
>
> - **Renamed CareRounds → CareRounds** (brand at carerounds.care, under
>   Juno Code Studio). The code rename is **done** — Dart identifiers,
>   `pubspec name:`, bundle id `com.carerounds.carerounds`, and the
>   `carerounds://` scheme are all CareRounds. Wherever this spec says
>   "CareRounds," read "CareRounds." (The repo *directory* is still named
>   `carerounds`, and a few internal ids — `carerounds_user_id`, the
...
>   free-text model.
> - **All Dr. Natali / Dementia CareRounds framing is removed** — the
...
grounded in Dr. Natali Edmonds' (Dementia CareRounds) coaching
...
- **Care-circle connect** (2026-06-06 — username + QR connect, §13
  ...
  - `mobile_scanner` 6.0+ — reads another caregiver's QR on the "Scan to
    ...
    `carerounds:circle:<token>` so the scanner validates the code is
  ...
  - `app_links` 6.3+ (2026-06-08 — invite LINKS) — receives the
    ...
    link: `carerounds://join/<token>` (plus the QR-style
    `carerounds:circle:<token>` for symmetry). Handles BOTH cold-start
    ...
    `carerounds` scheme in `ios/Runner/Info.plist`; Android registers it
...
## 3. Brand identity (sourced from carerounds.com, 2026-05-28)
...
| `cta` | `#C97458` | Primary action button (decoder result CTAs, send buttons, the center mic). Salmon — a deliberate rebrand to match carerounds.com (commit 33d497f); the original spec value was `#ff6900` orange. |
...
- App display name: `CareRounds`.
- Bundle ID: `com.carerounds.app`.
...
- AppBar: title "CareRounds" (`headlineMedium`), top-right `⚙` gear that pushes `/settings`.
...
- Three full-width outcome buttons stacked:
  ...
  - 💬 **I need to talk to Natali** — light secondary, opens external URL `https://carerounds.com/care-collective?utm_source=app&utm_medium=decoder` in in-app browser.
...
> re-homed under **Community → Learn** (the CareRounds content
...
- Top: "CareRounds" wordmark + tagline.
- Two buttons:
...
CareRounds content library, and caregiver-wellbeing/support
...
- **Feed** — the social experience. A scroll of posts; each post:
  ...
  **word-labeled actions: Like · Comment · Share** (word labels, not
  ...
  CareRounds posts and caregiver posts; supports grouping by topic /
  disease stage. (Tapping a post → post detail, level 2, with
  path-header `Home › Community` + word-labeled Back.)
- **Learn** — the CareRounds content library: framework videos and
...
- `FakeAuthProvider`: skips OAuth, returns a canned User
  `{email: 'demo@carerounds.app', name: 'Sarah Henderson'}` on
...
CareRounds). Your output is a script the caregiver can use IN THE
...
```python
#!/usr/bin/env python3
"""Local HTTP shim that wraps `claude --print` for the CareRounds
...
def main():
    print(f"[shim] CareRounds LLM shim listening on http://localhost:{PORT}")
...
page 3: *"CareRounds is not a medical product. It's a coaching tool
...
- App uses the name "CareRounds" — Natali's audience-identifier.
