...
async function seedProfile(overrides: Partial<typeof profiles.$inferInsert> = {}) {
  const [row] = await db()
    ...
    .values({
      ...
      careroundsUserId: `cb-${crypto.randomUUID()}`,
      ...overrides,
    })
    .returning();
  return row;
...
describe('forum schema round-trip', () => {
  ...
  describe('profiles', () => {
    it('round-trips a profile with defaults', async () => {
      ...
      expect(readBack.careroundsUserId).toBe(profile.careroundsUserId);
    });

    it('rejects a duplicate carerounds_user_id', async () => {
      const first = await seedProfile();
      await expect(
        seedProfile({ careroundsUserId: first.careroundsUserId }),
      ).rejects.toThrow();
    });
  });
