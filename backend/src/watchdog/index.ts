...
export function buildEmail(
  ...
  const subject = severity === 'red'
    ? `[CareRounds] RED: ${flags.length} metric${flags.length === 1 ? '' : 's'} at 75%+ of cap`
    : `[CareRounds] YELLOW: ${flags.length} metric${flags.length === 1 ? '' : 's'} at 50%+ of cap`;

  const lines: string[] = [];
  lines.push(`CareRounds weekly watchdog — ${generatedAt.toISOString()}`);
  lines.push('');
