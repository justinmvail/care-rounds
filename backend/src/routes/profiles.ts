...
// Deterministic 6-hex-char hash of the carerounds_user_id so a retried
// bootstrap call produces the same default name.
async function defaultDisplayName(careroundsUserId: string): Promise<string> {
  const bytes = new TextEncoder().encode(careroundsUserId);
...
function meResponse(profile: Profile) {
  return {
    id: profile.id,
    carerounds_user_id: profile.careroundsUserId,
...
// circle data; `careroundsUserId` is the JWT `sub` that keys llm_usage.
async function deleteAccount(
  db: Db,
  blobs: R2Bucket,
  media: R2Bucket,
  profileId: string,
  careroundsUserId: string,
  ...
  // --- LLM usage ledger (keyed by the JWT sub / carerounds_user_id) ---
  const deletedUsage = await db
    .delete(llmUsage)
    .where(eq(llmUsage.userId, careroundsUserId))
...
export const profilesRouter = () => {
  ...
  router.post('/bootstrap', async (c) => {
    const careroundsUserId = c.get('userId');
    const db = drizzle(c.env.FORUM_DB);

    const [existing] = await db
      .select()
      .from(profiles)
      .where(eq(profiles.careroundsUserId, careroundsUserId));
    if (existing) {
      return c.json(meResponse(existing), 200);
    }

    const displayName = await defaultDisplayName(careroundsUserId);
    const [created] = await db
      .insert(profiles)
      .values({ displayName, careroundsUserId })
      .returning();
      ...
      const [taken] = await db
        ...
        .where(
          and(
            ...
            ne(profiles.careroundsUserId, careroundsUserId),
          ),
        );
      if (taken) {
