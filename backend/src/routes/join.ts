...
/// `carerounds://join/<token>` deep link that hands off to the app
...
export function joinRouter() {
  ...
  router.get('/:token', (c) => {
    ...
    const deepLink = `carerounds://join/${safe}`;
    const html = `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1" />
<title>You're invited to a care circle on CareRounds</title>
</head>
...
<h1 style="font-size:24px;line-height:1.3;margin:0 0 16px;">You're invited to a care circle on CareRounds</h1>
...
<a href="${deepLink}" style="display:block;text-align:center;background:#ff6900;color:#ffffff;text-decoration:none;font-size:18px;font-weight:700;padding:16px 20px;border-radius:14px;margin:0 0 28px;">Open in CareRounds</a>
