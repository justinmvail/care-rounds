# CareRounds Forum Backend

Cloudflare Workers backend for the CareRounds caregiver forum
...
- `/join/*` (root, NOT under `/api/v1`) — the public care-circle invite
  ...
  and a `carerounds://join/<token>` deep link into the app.
...
(`media.carerounds.local`) that resolved nowhere and there was no upload route at
...
> as the `com.carerounds.worker` LaunchAgent (LIVE tester backend,
...
hard-deleted afterwards; it refuses to run against `carerounds.care` unless
...
wrangler login
wrangler d1 create care-rounds-forum
#   → paste the returned UUID into wrangler.toml's database_id
wrangler r2 bucket create care-rounds-forum-media   # FORUM_MEDIA — avatars + post images
wrangler r2 bucket create carerounds-doc-blobs     # DOC_BLOBS — caregiver document scans
...
#     APPLE_BUNDLE_ID       — the app bundle id (com.carerounds.carerounds)
#     GOOGLE_PLAY_SA_EMAIL  — the Play service-account email
#     GOOGLE_PLAY_PACKAGE   — the Android package (com.carerounds.carerounds)
...
A red flag promotes the email subject to `[CareRounds] RED: …`;
otherwise it ships as `[CareRounds] YELLOW: …`. Red takes
...
- `CLOUDFLARE_R2_BUCKET_NAME` — defaults to `care-rounds-forum-media`
...
`GET /health` (`https://care-rounds-forum.jcsvonellc.workers.dev/health`, returns
