...
| # | Bug | Effect |
|---|-----|--------|
| 1 | Every call to `CareRoundsDatabase.open()` eagerly built a throwaway executor, each of which raced to mint its own SQLCipher key | The DB ended up encrypted under a key nobody kept → `file is not a database` on every launch → **care record destroyed** |
...
`CareRoundsDatabase.open()` returns a **process-wide singleton** — one connection
