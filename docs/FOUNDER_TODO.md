...
_The human-only work to get CareRounds to production + through the ACL Challenge.
...
- [ ] **GitHub** — create the JCSV org and **transfer the `carerounds` repo** to it (repo → Settings → Transfer). Tell Claude when done; he updates the git remote + docs. *(ACCOUNT)*
...
- [ ] **Create iOS + Android OAuth clients** for `com.carerounds.carerounds` (steps in `OPERATOR_SETUP.md §1`). **Paste the iOS client id to Claude** to wire it. *(CONSOLE)*
...
> environment → **`care-rounds-forum-dev.jcsvonellc.workers.dev`**. Live +
> verified: Worker, D1 `care-rounds-forum-dev` (9 migrations), JWT auth, and the
...
> (buckets `care-rounds-forum-media-dev` + `carerounds-doc-blobs-dev` bound as
...
> it via `FORUM_API_URL=https://care-rounds-forum-dev.jcsvonellc.workers.dev`.
> The items below are the remaining **production** (carerounds.care) deploy.
- [x] **Enable R2** — done 2026-07-10 (account-wide). Dev buckets `care-rounds-forum-media-dev` + `carerounds-doc-blobs-dev` created + bound. Prod buckets (`care-rounds-forum-media` / `carerounds-doc-blobs`) still to create at prod deploy. *(CONSOLE/$)*
- [ ] Create buckets `care-rounds-forum-media` + `carerounds-doc-blobs`. *(CONSOLE)*
...
- [ ] **Point `carerounds.care` DNS at the Worker** (add the zone to Cloudflare, attach the custom domain). Required for the Terms/Privacy pages + invite links. *(CONSOLE)*
...
- [ ] **Android release keystore** — generate with a secure password (Corretto-17 keytool) → `android/carerounds-release.keystore` + `android/key.properties`; **register its SHA-1 on the OAuth Android client** (steps in `OPERATOR_SETUP.md §4.2`). *(CONSOLE)*
- [ ] **Apple signing** — App ID `com.carerounds.carerounds`, distribution cert, provisioning profile. *(CONSOLE)*
...
- [~] **Play org account "Juno Code Studio"** — Organization account, ID 5351202474101549368. $25 + org creation done; org accounts SKIP the closed-testing gate. Verification progress (2026-07-10):
  ...
  - ✅ **Website** — verified `junocode.studio` (the live JCSV/Juno Code Studio site, on Cloudflare — NOT carerounds.care, so it was never blocked on the DNS work) via **Google Search Console** ownership (DNS TXT).
...
- [ ] Create the CareRounds app (no closed-testing period needed on an org account). *(CONSOLE)*
- [ ] **Data safety form** (mirror the iOS labels). *(PERSONAL)*
- [ ] Content rating (IARC); target audience; ads declaration (none); public privacy URL `carerounds.care/privacy`. *(PERSONAL)*
...
7. **Cloudflare deploy** — enable R2, secrets (incl. `CF_AI_API_TOKEN`), `npm run deploy`, migrate D1, point `carerounds.care` DNS at the Worker.
