...
> **Note (2026-06-22 pivot):** the app is rebranding **CareRounds →
> CareRounds** and going **general-purpose caregiving** (not dementia-only).
> The IA below (4-tab bar, hubs, path header) is still accurate; just read
> "CareRounds" as "CareRounds," and note the Behavior Decoder is being
...
- **Feed** — the social experience: posts with avatar, name, timestamp, body text, optional image, and **word-labeled actions (Like · Comment · Share)**. Includes official CareRounds posts and caregiver posts; supports grouping by topic / care situation.
