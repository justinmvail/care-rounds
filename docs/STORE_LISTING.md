# App Store Submission Content — CareRounds
...
> `carerounds.care/privacy` (source: `backend/src/routes/legal.ts`). If the
...
Brand / support: **carerounds.care** · support@carerounds.care

---

## 1. App Store (iOS) — App Privacy "Nutrition Labels"

Fill these into **App Store Connect → App Privacy**. CareRounds collects the
...
```
Caring for someone you love is a lot to hold. CareRounds keeps it in one
...
CareRounds also cuts the busywork of coordination. Scan a prescription label,
...
choose to share it. There are no ads, and we never sell your data. CareRounds
...
**Medical.** CareRounds is a medication- and care-coordination tool with health
...
### Support URL
`https://carerounds.care` (support: support@carerounds.care)
Privacy policy: `https://carerounds.care/privacy` · Terms:
`https://carerounds.care/terms`
...
- [ ] **Apple 5.1.1(v) — account deletion.** In-app deletion is shipped at
      **Settings → Delete account** (`SettingsScreen.deleteAccountButtonKey`;
      ...
      Safety deletion path: same in-app control + support@carerounds.care.
- [ ] **Medical disclaimer present.** The chat carries a persistent, code-side
      ...
      Privacy pages restate that CareRounds is **not a medical device** and does
...
- [ ] **Export compliance.** CareRounds uses only standard encryption
...
- [ ] **Privacy declarations match this doc + the live policy.** App Store
      ...
      `carerounds.care/privacy`. Re-check after any `legal.ts` change.
...
- [ ] **TODO (operator, pre-submit):** attorney review of Terms + Privacy
      ...
      CareRounds values in the release build; ensure the release LLM backend is
