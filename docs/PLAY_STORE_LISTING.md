# Google Play Console content — CareRounds

Draft to paste into Play Console, grounded in the actual app (permissions,
backend tables, privacy policy at junocode.studio/carerounds/privacy.html).
Everything here is accurate as of build 0.1.0+51. **Where a field is a judgment
call, it's flagged ⚠.**

Publisher: **JCSV One LLC, dba Juno Code Studio** · Package: `com.carerounds.carerounds`
Support email: **jcsvonellc@gmail.com** · Privacy policy:
**https://junocode.studio/carerounds/privacy.html**
...
**App name** (≤30 chars)
```
CareRounds: Caregiver Coach
```
(26 chars. Alternative if you want the name alone: `CareRounds`.)
...
details only you seem to hold. CareRounds keeps them in one place, and gives you
a coach that actually knows your person's situation.

Unlike a blank chat box, CareRounds's coach is grounded in the real details you
...
CareRounds is general-purpose caregiving — for anyone caring for someone who
...
CareRounds is a wellbeing tool for caregivers — not a medical service, and not a
...
https://junocode.studio/carerounds · (phone optional).
...
it to a third party for *their* use). CareRounds's backend runs on **Cloudflare**
...
- **Target age group:** **18 and over.** CareRounds is for adult caregivers; it
...
- **Data deletion URL** (Play now asks for one): use the in-app path (Settings →
  ...
  junocode.studio/carerounds/delete page if you want a public URL.
