...
> **CareRounds → CareRounds**, and any "Dr. Natali voice" catalog mention
> is obsolete (the Dr. Natali / Dementia CareRounds branding is being
...
- `ios/Runner/TTSBridge.swift` — owns the `ORTSession`, AVAudioEngine
  ...
  protocol. The bridge registers the `carerounds/tts` MethodChannel
...
> any release build has `CAREROUNDS_HAS_ESPEAK_NG=1` and `useEspeak:
> true`.
...
3. The shipping UI has no tap-to-play button yet. The Behavior Decoder
   that hosted the old 🔊 PLAY affordance was removed in the CareRounds
...
- `android/app/src/main/kotlin/com/carerounds/carerounds/TTSBridge.kt`
  ...
  `carerounds/tts` MethodChannel and handles `speak`, `cancel`,
  `availableVoices`.
- `android/app/src/main/kotlin/com/carerounds/carerounds/MainActivity.kt`
  — replaces the Phase 9.2 stub with a single call to
  `TTSBridge.register(applicationContext, flutterEngine)`.
- `android/app/src/androidTest/kotlin/com/carerounds/carerounds/TTSBridgeInstrumentedTest.kt`
...
> (`libcarerounds_espeak_ng.so`) when `EspeakNGNative.isAvailable` is
...
- **Bridge unavailable.** The native plugin failed to register —
  ...
  link. The `carerounds/tts` MethodChannel surfaces a
...
- **Probe throws `PlatformException`** → factory logs a single
  WARN line via `dart:developer` (`name: 'carerounds.tts'`,
...
- `ios/Runner/Runner-Bridging-Header.h` — adds a `__has_include`-
  ...
  The guard sets `CAREROUNDS_HAS_ESPEAK_NG` to 1 when the headers
...
- `ios/RunnerTests/RunnerTests.swift` — adds
  ...
  output is non-empty. Skips when `CAREROUNDS_HAS_ESPEAK_NG == 0`
...
- `android/app/src/main/cpp/carerounds_espeak_ng.cpp` — JNI shim
...
After the first build, `libcarerounds_espeak_ng.so` ships in the
...
1. **No internal Maven yet.** Setting one up (or signing into the
   carerounds org's GitHub Packages registry) is real infrastructure
...
platform (so `CAREROUNDS_HAS_ESPEAK_NG=1` on iOS and
...
- **iOS**: `xcodebuild test -only-testing:RunnerTests/RunnerTests/testRegenerateAudioQualitySamples`
  ...
  and writes 16-bit-PCM WAVs into `NSTemporaryDirectory()/carerounds-tts-samples/<voice>/`.
...
- **Android**: `./gradlew :app:connectedDebugAndroidTest -P...class=...#regenerateAudioQualitySamples`
  ...
  `adb pull /sdcard/Android/data/com.carerounds.carerounds/files/tts_samples/<voice>/<slug>.wav`
