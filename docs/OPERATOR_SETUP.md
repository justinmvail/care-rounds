...
The rebrand changed the app id from `com.carerounds.carerounds` to
`com.carerounds.carerounds`. OAuth iOS/Android clients are bound to that id,
...
3. The old `com.carerounds.carerounds` iOS/Android clients can be deleted
...
```bash
npx wrangler r2 bucket create care-rounds-forum-media   # FORUM_MEDIA
npx wrangler r2 bucket create carerounds-doc-blobs     # DOC_BLOBS
...
## 3. carerounds.care routing

`carerounds.care` currently has no DNS records. For the Worker to serve the
...
domain to the `care-rounds-forum` Worker (Workers & Pages → care-rounds-forum →
Settings → Domains & Routes → `carerounds.care`). The app already links to
`https://carerounds.care/terms` + `/privacy`; those 404/NXDOMAIN until this
step.
...
1. Apple Developer Program + Google Play Console **organization**
   ...
   the Desktop; publisher DBA must not be "CareRounds".
2. Real release signing for Android — the signing **config** is now wired
   ...
   password (Corretto-17 keytool), put it at `android/carerounds-release.keystore`
