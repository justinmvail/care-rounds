# CareRounds — Roadmap to Production
...
- [ ] 👤 **Create the JCSV GitHub org/account** and **migrate the `carerounds` repo** to it (GitHub repo → Settings → Transfer ownership; GitHub redirects the old `justinmvail/carerounds` URL). Then `git remote set-url origin` locally + update `CLAUDE.md`/memory. Move any CI/secrets with it.
...
- [ ] 👤 **Google auth under the JCSV Google account** — own the GCP project (187697773608) from `jcsvonellc@gmail.com` (grant Owner, accept, then create the new `com.carerounds.carerounds` OAuth clients from the LLC account per `OPERATOR_SETUP.md §1`). One identity for OAuth, Play, and payout.
...
### 1a. Identity & sign-in
- [ ] 🖥️ **Create Google OAuth iOS + Android clients for `com.carerounds.carerounds`** (project 187697773608). Sign-in is broken in `AUTH=google` builds until this exists. Steps in `docs/OPERATOR_SETUP.md §1`. → paste the iOS client id to Claude to wire `Info.plist` + `dev_defines.sh`.
...
- [ ] 🖥️ From `backend/`: `npx wrangler r2 bucket create care-rounds-forum-media` and `… carerounds-doc-blobs`.
...
- [ ] 🖥️ **Custom domain:** attach `carerounds.care` (and/or `api.carerounds.care`) to the Worker (Workers & Pages → care-rounds-forum → Domains & Routes). Add the zone to Cloudflare first — no DNS records exist yet.
...
- [ ] 🖥️ **Android release keystore:** generate one (secure password, Corretto-17 keytool) → `android/carerounds-release.keystore` + `android/key.properties` (both gitignored; `build.gradle.kts` already reads them, debug-fallback when absent). Register its **SHA-1 on the OAuth Android client**. Steps in `OPERATOR_SETUP.md §4.2`.
- [ ] 👤 **Apple signing:** App ID `com.carerounds.carerounds`, distribution certificate, App Store provisioning profile (Xcode-managed is fine).

### 1e. App Store (iOS) — submission runbook
- [ ] 👤 In App Store Connect: create the app record (bundle id `com.carerounds.carerounds`), name "CareRounds", primary category, subtitle.
...
- [ ] 🤝 Store listing: description, keywords, support URL (`carerounds.care`), marketing URL, promotional text, 6.5"/5.5" screenshots + iPad if universal, optional app preview.
...
- [ ] 👤 Content rating (IARC) questionnaire; target-audience; ads declaration (none); a **public privacy-policy URL** = `carerounds.care/privacy` (needs 1b DNS live).
