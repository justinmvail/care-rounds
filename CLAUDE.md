# Claude Project Context — CareRounds
...
> The product began as "CareRounds"; it is now **CareRounds**. The code is
> fully migrated: the Behavior Decoder is gone, all Dr. Natali / dementia-
> specific framing is stripped, and `CareRounds*` identifiers + the
> `carerounds://` scheme + the `pubspec name:` are renamed to CareRounds
> (`CareRoundsColors`, `context.hc`, `CareRoundsDatabase`, `carerounds://`,
> `package:carerounds/…`, bundle id `com.carerounds.carerounds`). The repo
> directory and GitHub repo were renamed too (2026-07-08): the repo is
> **`/Users/jvail/IdeaProjects/carerounds`**, remote
> **`github.com/justinmvail/carerounds`**. A few internal identifiers are
> **deliberately kept** as `carerounds` and must NOT be "fixed": the
> `carerounds_user_id` D1 column / JSON wire key, the Android
> `namespace` + `com.carerounds.carerounds` Kotlin/JNI package + native
> espeak lib, the `com.carerounds.*` LaunchAgent labels + log filenames
...
**CareRounds** is a Flutter mobile app — iOS + Android — that gives family
...
Published under **Juno Code Studio** (JCSV One LLC) at **carerounds.care**.

## Direction (pivot — completed 2026-06-23)

The product began as "CareRounds," an in-the-moment dementia-behavior
decoder, built as a partnership pitch to Dr. Natali Edmonds (Dementia
CareRounds). **That pitch went unanswered**, so the app was de-branded and
shipped as its own product, **CareRounds** ("hold the people you love
close"), brand at **carerounds.care**. The three-phase pivot is **done and
...
2. **Coach re-voiced general-purpose** — Dr. Natali / Dementia CareRounds
...
3. **Renamed CareRounds → CareRounds** — Dart identifiers, brand strings,
   l10n, `pubspec name:` (`package:carerounds/…`), iOS+Android display name +
   bundle id (`com.carerounds.carerounds`), the `carerounds://` deep-link
   scheme, the `carerounds/tts` method channel, and the backend
...
fresh app; old `carerounds://` invite links won't open it); create the
CareRounds R2 buckets on deploy (D1 binds by id, so no data migration).
...
```
carerounds/                  # repo root (matches pubspec name:)
  ...
  lib/
    ...
    theme.dart              # brand tokens (navy #1f2a44, salmon CTA #C97458,
                            # warm white #f8f6f3) via CareRoundsColors/context.hc
...
:8765** (LaunchAgent `com.carerounds.shim`, Funnel-exposed) — scratch
...
- Don't make the app talk to `api.anthropic.com` directly in dev — use
  the shim.
- Don't reintroduce **any** Dr. Natali / Dementia CareRounds branding,
  attribution, or copy — the product is its own brand (CareRounds) now.
...
- Don't ship default Material colors. The brand tokens in `lib/theme.dart`
  ...
  (the `CareRoundsColors` theme extension), never raw hex. (Palette carried
  over from CareRounds; may be revisited for the CareRounds brand, but it's
