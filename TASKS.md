...
- [x] **Task 1: Flutter project init + folder structure.** Run `flutter
  create carerounds --org com.carerounds --platforms ios,android` and
...
- [x] **Task 2: Brand theme (`lib/theme.dart`).** Implement the brand
  ...
  Exported names: `careroundsLightTheme`, `careroundsDarkTheme`,
  `careroundsColors` (a `CareRoundsColors` data class holding the
  ...
  in `CareRoundsColors` is the exact hex from BUILD_SPEC.md;
  `careroundsLightTheme.colorScheme.primary` returns the navy;
...
- [x] **Task 3: App scaffold + routing (`lib/app.dart`, `lib/main.dart`,
  ...
  `careroundsLightTheme` + dark theme + system-mode fallback. Build
...
- [x] **Task 14: Decoder result screen
  ...
  the carerounds.com URL via `url_launcher` (add as dep if not
...
- [x] **Task 20: PDF exporter (`lib/services/pdf_exporter.dart`).**
  ...
  worked + notes. Footer: "Generated by CareRounds. Not a substitute
...
- [x] **Task 24: Crisis card screen
  ...
  encoding `https://carerounds.app/patient/{patient.id}` (placeholder
...
`assets/tts/<voice-id>/` — carerounds consumes the `.onnx` artifact
...
- [x] **Phase 9.2: Dart-side `BundledTTSProvider` skeleton +
  ...
  `TTSProvider`. Bridge to a new `carerounds/tts` MethodChannel
...
- [x] **Phase 9.4: Android Kotlin bridge — ONNX Runtime + NNAPI EP +
  ...
  `android/app/src/main/kotlin/com/carerounds/carerounds/TTSBridge.kt`:
...
levels). Native UI inside carerounds. Cloudflare hosting chosen for
...
Stack:
  ...
  - **`backend/`** directory in the repo holds the Worker code;
    ...
    carerounds Apple/Google sign-in. No separate auth system.
...
- [x] **Phase 13.2: Drizzle schema + migrations
  ...
  per the spec:
    - `profiles` (id UUID, display_name TEXT, avatar_url TEXT,
      joined_at, role TEXT default 'user', carerounds_user_id TEXT
...
- [x] **Phase 13.3: Hono API skeleton + JWT auth middleware
  ...
  put FORUM_JWT_SECRET`). Token claims: `{sub: carerounds_user_id,
...
- [x] **Phase 13.4: Profile bootstrap + endpoints
  (`backend/src/routes/profiles.ts`).** Four endpoints:
    - `POST /api/v1/profiles/bootstrap` — called once per carerounds
      user after first sign-in. Creates a profile row keyed on
      carerounds_user_id. Default display_name is
...
- [x] **Phase 14.2: PathHeader widget (`lib/widgets/path_header.dart`
  ...
  Brand tokens: navy text, `careroundsColors.primarySoft` for `›`,
...
- [x] **Phase 14.3: HubTile + HubGrid widgets (`lib/widgets/hub_tile.dart`
  ...
  `careroundsColors` (the HTML's coral/teal/amber/plum are
...
- [x] **Phase 14.5: Rewrite `lib/routing/router.dart` shell branches.**
  ...
  CommunityFeedScreen. Add new route names to `CareRoundsRoutes`:
...
- [x] **Phase 14.28: Care Circle invite form
  ...
  composes a share message ("Join my CareRounds care circle:
  https://carerounds.app/invite/<token>") and either uses
...
- [x] **Phase 14.37: Learn segment + content seeds
  ...
  sub-section "Videos" = a vertical list of seeded CareRounds
...
- Uses `testWidgets` from `flutter_test` + the shared harness from
  15.1 to pump the full `CareRoundsApp` widget with fake providers,
...
- [x] **Phase 15.1: Shared integration harness
  ...
  every Phase 15 `*_flow_test.dart`. Exposes:
    - `Future<ProviderContainer> pumpCareRoundsApp(WidgetTester
      ...
      `CareRoundsDatabase.testInstance()`, fixed clock
      ...
      `CareRoundsApp(...)`, settles, returns the container.
...
- [x] **Phase 15.2: Drift test instance factory
  ...
  `CareRoundsDatabase.testInstance()` returning a fresh database
