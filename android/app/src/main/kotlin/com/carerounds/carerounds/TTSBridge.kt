package com.carerounds.carerounds
...
// BUILD_SPEC.md Phase 9.4 — `carerounds/tts` MethodChannel handler.
//
...
object TTSBridge {
    ...
    const val CHANNEL_NAME: String = "carerounds/tts"
...
class TTSEngine(private val appContext: Context) {
    ...
    private val workExecutor = Executors.newSingleThreadExecutor { runnable ->
        Thread(runnable, "carerounds.tts.work").apply { isDaemon = true }
    }
    ...
    private val playExecutor = Executors.newSingleThreadExecutor { runnable ->
        Thread(runnable, "carerounds.tts.play").apply { isDaemon = true }
    }
...
/// `libcarerounds_espeak_ng.so` by the externalNativeBuild config in
...
/// Singleton wrapper around `libcarerounds_espeak_ng.so`. The C++
/// shim lives at `android/app/src/main/cpp/carerounds_espeak_ng.cpp`;
...
object EspeakNGNative {
    ...
    init {
        var available = false
        try {
            System.loadLibrary("carerounds_espeak_ng")
            available = nativeHasEspeakNG()
        } catch (e: UnsatisfiedLinkError) {
            Log.w("TTSBridge", "libcarerounds_espeak_ng not loadable — bridge falls back to character phonemizer", e)
        }
    ...
    /// (-1/-2/-3, see carerounds_espeak_ng.cpp) on failure.
