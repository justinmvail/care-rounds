...
import 'package:carerounds/services/billing_service.dart';
...
void main() {
  ...
  group('CareRoundsProductIds', () {
    test('bundles both plan ids into `all`', () {
      expect(
        CareRoundsProductIds.all,
        <String>{CareRoundsProductIds.monthly, CareRoundsProductIds.annual},
      );
      // Placeholders live under the app's bundle namespace so they read
      // clearly in App Store Connect / Play Console.
      expect(CareRoundsProductIds.monthly, startsWith('com.carerounds.'));
      expect(CareRoundsProductIds.annual, startsWith('com.carerounds.'));
    });
  });
  ...
  group('FakeBillingService', () {
    ...
    test('loadProducts returns both canned plans, each carrying a free trial',
      ...
      expect(
        ...
        containsAll(<String>[
          CareRoundsProductIds.monthly,
          CareRoundsProductIds.annual,
        ]),
      );
  ...
  group('StoreBillingService — server-verified entitlement', () {
    test('a completed purchase POSTs the receipt and premium reflects the '
      ...
      final _FakeEntitlementApi api = _FakeEntitlementApi(
        ...
        verifyResult: const ServerEntitlement(
          isPremium: true,
          productId: 'com.carerounds.premium.monthly',
          platform: 'ios',
        ),
      );
      ...
      iap.emit(<PurchaseDetails>[
        _purchase('com.carerounds.premium.monthly', receipt: 'jws-abc'),
      ]);
      await _settle();
    ...
    test('a completed purchase does NOT grant when the SERVER says '
      ...
      await svc.initialize();
      iap.emit(<PurchaseDetails>[_purchase('com.carerounds.premium.monthly')]);
      await _settle();
  ...
  group('ServerEntitlement', () {
    test('round-trips through JSON and equals', () {
      const ServerEntitlement e = ServerEntitlement(
        ...
        productId: 'com.carerounds.premium.annual',
        platform: 'android',
      );
