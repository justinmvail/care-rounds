...
import 'package:carerounds/db/tables.dart';
import 'package:carerounds/models/appointment.dart';
import 'package:carerounds/models/health_log_entry.dart';
...
import 'package:carerounds/models/document.dart';
import 'package:carerounds/models/expense.dart';
...
import 'package:carerounds/services/data_exporter.dart';
...
ExportSources _sourcesFor(CareRoundsDatabase db, StorageProvider storage) => (
      storage: storage,
...
void main() {
  ...
  // back over the `carerounds/document_import` channel's `pickJson` method —
  ...
  group('RealDataFilePicker — carerounds/document_import wire contract', () {
    const MethodChannel channel = MethodChannel('carerounds/document_import');
  ...
  group('DataExporter.gather — Issue #20', () {
    ...
    test(
      ...
      () async {
        ...
        final CareRoundsDatabase dstDb =
  ...
  group('DataExporter.exportAndShare — Issue #20', () {
    test('hands a date-stamped JSON file to the sharer', () async {
      ...
      expect(filename, 'carerounds-backup-2026-06-04.json');
  ...
  group('DataExporter.importInto — Issue #20 (round-trip restore)', () {
    test(
      ...
      () async {
        ...
        final CareRoundsDatabase srcDb =
            CareRoundsDatabase(NativeDatabase.memory());
