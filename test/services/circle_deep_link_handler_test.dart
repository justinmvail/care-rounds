...
import 'package:carerounds/services/circle_deep_link_handler.dart';
import 'package:carerounds/services/circle_invite_link.dart';
...
void main() {
  group('parseCircleInviteToken', () {
    test('parses the link-style carerounds://join/<token>', () {
      expect(
        parseCircleInviteTokenFromString('carerounds://join/abc123'),
        'abc123',
      );
    });

    test('parses the QR-style carerounds:circle:<token>', () {
      expect(
        parseCircleInviteTokenFromString('carerounds:circle:xyz789'),
        'xyz789',
      );
    });
    ...
    test('returns null for an unrelated / empty URI', () {
      ...
      expect(parseCircleInviteTokenFromString('carerounds://join/'), isNull);
  ...
  group('CircleDeepLinkHandler', () {
    test('valid token while signed in → asks for CONFIRMATION, never '
      ...
      final outcome =
          await handler.handleUri('carerounds://join/good_token');
    ...
    test('signed out → stashes; replay after sign-in STILL requires '
      ...
      final stashed = await handler.handleUri('carerounds://join/good_token');
