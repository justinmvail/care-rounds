...
import 'package:carerounds/services/sync_service.dart' show SyncOutbox;
...
/// `CareRoundsDatabase` connection to the SAME on-disk file. Once
...
///   1. [CareRoundsDatabase.open] returns a process-wide SINGLETON, so the
...
void main() {
  group('shared single DB connection (the real lock fix)', () {
    tearDown(CareRoundsDatabase.resetSharedForTest);
    ...
    test('open() returns the SAME instance every call (one connection)', () {
      ...
      final CareRoundsDatabase a = CareRoundsDatabase.openShared(
          NativeDatabase(File('${tmp.path}/a.sqlite')));
      final CareRoundsDatabase b = CareRoundsDatabase.openShared(
    ...
    test('open() constructs its executor LAZILY — no throwaway executors',
      ...
      final CareRoundsDatabase a =
          CareRoundsDatabase.open(createExecutor: buildExecutor);
      final CareRoundsDatabase b =
          CareRoundsDatabase.open(createExecutor: buildExecutor);
    ...
    test('a provider disposing it is a no-op — the shared connection lives on',
      ...
      final CareRoundsDatabase db = CareRoundsDatabase.openShared(
