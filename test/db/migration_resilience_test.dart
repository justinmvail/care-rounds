...
void main() {
  ...
  setUp(() {
    ...
    dbPath = '${tmp.path}/carerounds.sqlite';
  });
  ...
  Future<void> materialiseSchema() async {
    final CareRoundsDatabase db =
        CareRoundsDatabase(NativeDatabase(File(dbPath)));
  ...
  group('a schema with NO version stamp is repaired, not recreated', () {
    test('user_version 0 + our tables present → stamped, and the app opens',
      ...
      stampSchemaVersionIfMissing(dbPath, 22);

      expect(versionOf(dbPath), 22, reason: 'the stamp must be restored');
