...
/// (TASKS.md Phase 15.1). Pumps the full [CareRoundsApp] over fake
...
///
/// Every flow test calls [pumpCareRoundsApp] first, optionally seeds via
...
import 'package:carerounds/models/appointment.dart';
import 'package:carerounds/models/journal_entry.dart';
import 'package:carerounds/models/medication.dart';
import 'package:carerounds/providers/analytics_provider.dart';
...
import 'package:carerounds/providers/llm_provider.dart';
import 'package:carerounds/providers/onboarding_provider.dart';
import 'package:carerounds/providers/patient_configured_provider.dart';
import 'package:carerounds/providers/quiet_hours_provider.dart';
import 'package:carerounds/providers/storage_provider.dart';
import 'package:carerounds/providers/tts_provider.dart';
...
import 'package:carerounds/seed/sample_journal.dart';
...
///
/// Wired in as the [linkLauncherProvider] override so the Team
...
/// Pump the full [CareRoundsApp] wired for an integration flow and
...
Future<ProviderContainer> pumpCareRoundsApp(
  WidgetTester tester, {
  DateTime? clock,
  ...
  // leak a teardown timer). [CareRoundsDatabase.testInstance] wraps a
  ...
  // `CareRoundsDatabase.open()` handle here; in the harness we share the
  ...
  // `CareRoundsDatabase.open()` (real on-device SQLite) and throw inside the
  ...
  final ProviderContainer container = ProviderScope.containerOf(
    tester.element(find.byType(CareRoundsApp)),
    listen: false,
  );
  ...
  if (initialLocation != '/') {
    container.read(careroundsRouterProvider).go(initialLocation);
    await tester.pumpAndSettle();
  }
