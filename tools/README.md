...
How to build/run CareRounds on a device, every build flag, and where each
...
~/Library/LaunchAgents/<label>.plist`; `launchctl list | grep carerounds`
to see them). The labels + log filenames keep the historical
`carerounds` naming deliberately:

| Label | What it runs | Port → Funnel | Log |
|---|---|---|---|
| `com.carerounds.shim` | `claude_shim.py` (`SHIM_TOKEN` lives in the plist) | 127.0.0.1:8765 → public **:443** | `~/Library/Logs/carerounds-shim.log` |
| `com.carerounds.worker` | `npm run dev` in `backend/` (wrangler dev) | 127.0.0.1:8787 → public **:8443** | `~/Library/Logs/carerounds-worker.log` |
| `com.carerounds.funnelcert` | `refresh_funnel_cert.sh` every 5 min (funnel cert + self-heal) | — | `~/Library/Logs/carerounds-funnel-cert.*.log` |
