...
# FORUM_API_URL is empty, so there is no backend. Every screen in CareRounds sits
# behind the sign-in gate (see careroundsRedirect in lib/routing/router.dart), so
...
case "$BACKEND" in
  cloudflare-prod)
    # The prod Worker's own origin. NOT carerounds.care — no DNS (2026-07-14).
...
GOOGLE_SERVER_CLIENT_ID="${GOOGLE_SERVER_CLIENT_ID:-REPLACE_WITH_CARE_ROUNDS_WEB_CLIENT_ID}"
