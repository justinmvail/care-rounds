#!/usr/bin/env bash
#
# tools/run_device.sh — the ONE script that builds + installs CareRounds to a
...
#                                          care-rounds-forum-dev.jcsvonellc.workers.dev
#                        cloudflare-prod = carerounds.care (once DNS is pointed).
#
# Examples:
  ...
  google)
    ...
    case "$BACKEND" in
      ...
      cloudflare|cloudflare-dev)
        FORUM_API_URL="https://care-rounds-forum-dev.jcsvonellc.workers.dev" ;;
      cloudflare-prod)
        # The prod Worker's own origin. NOT carerounds.care — that domain has no
        ...
        FORUM_API_URL="https://care-rounds-forum.jcsvonellc.workers.dev" ;;
      *)
...
echo "→ CareRounds build ${BUILD_NUMBER}  (auth=${AUTH}${SEED:++seed}, ${GIT_BRANCH} @ ${GIT_SHA}, ${BUILD_TIME})"
echo "  device=${DEVICE}"
